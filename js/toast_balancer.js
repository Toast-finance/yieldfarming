...
async function main() {
    ...
    const YGOV_BPT_POOL = new ethers.Contract(AVO_BPT_STAKING_POOL_ADDR, YGOV_BPT_STAKING_POOL_ABI, App.provider);
    const YFI_DAI_BALANCER_POOL = new ethers.Contract(AVO_DAI_BPT_TOKEN_ADDR, BALANCER_POOL_ABI, App.provider);
    const YFI_DAI_BPT_TOKEN_CONTRACT = new ethers.Contract(AVO_DAI_BPT_TOKEN_ADDR, ERC20_ABI, App.provider);
    ...
    const totalStakedBPTAmount = await YFI_DAI_BPT_TOKEN_CONTRACT.balanceOf(AVO_BPT_STAKING_POOL_ADDR) / 1e18;
    const totalYFIAmount = await YFI_DAI_BALANCER_POOL.getBalance(AVO_TOKEN_ADDR) / 1e18;
    ...
    const YFIIPrice = (await YFI_DAI_BALANCER_POOL.getSpotPrice(DAI_TOKEN_ADDR,AVO_TOKEN_ADDR) / 1e18) * DAIPrice;
    ...
    _print("========== PRICES ==========")
    _print(`1 AVO   = $${YFIIPrice}`);
    _print(`1 DAI   = $${DAIPrice}\n`);
    _print(`1 BPT   = [${YFIPerBPT} AVO, ${DAIPerBPT} DAI]`);
    _print(`        = ${toDollar(YFIPerBPT * YFIIPrice + DAIPerBPT * DAIPrice)}\n`);

    _print("========== STAKING =========")
    _print(`There are total   : ${totalBPTAmount} BPT issued by AVO DAI Balancer Pool.`);
    ...
    _print(`Weekly estimate   : ${toFixed(rewardPerToken * stakedBPTAmount, 2)} AVO = ${toDollar(rewardPerToken * stakedBPTAmount * YFIIPrice)} (out of total ${weekly_reward} AVO)`)
    ...
    _print(`Reward ends      : in ${forHumans(timeTilHalving)} \n`)
