...
async function main() {
    ...
    const stakedYAmount = await Y_STAKING_POOL.balanceOf(App.YOUR_ADDRESS);
    const earnedYFI = await Y_STAKING_POOL.earned(App.YOUR_ADDRESS) / 1e18;
    const totalStakedYAmount = await Y_TOKEN.balanceOf(ZZZ_POOL_B_ADDRESS);
    ...
    const {'toast-finance' : ReturnedPrice } = await lookUpPrices(["toast-finance"])
    ...
    _print("========== PRICES ==========")
    _print(`1 HOUSE = $${ZZZPrice}`);
    _print(`1 UNI   = $${UNIPrice}\n`);

    _print("========== STAKING =========")
    _print(`There are total   : ${totalStakedYAmount} HOUSE staked.`);
    _print(`                  = ${toDollar(totalStakedYAmount * ZZZPrice)}\n`);
    _print(`You are staking   : ${stakedYAmount} HOUSE (${toFixed(stakedYAmount * 100 / totalStakedYAmount, 3)}% of the pool)`);
    _print(`                  = ${toDollar(stakedYAmount * ZZZPrice)}\n`);

    // YFII REWARDS
    _print("======== UNI REWARDS ========")
    // _print(" (Temporarily paused until further emission model is voted by the community) ");
    _print(`Claimable Rewards : ${earnedYFI} UNI = $${toFixed(earnedYFI * UNIPrice, 2)}`);
    _print(`Weekly estimate   : ${rewardPerToken * stakedYAmount} UNI = $${toFixed(rewardPerToken * stakedYAmount * UNIPrice, 2)} (out of total ${weekly_reward} UNI)`)
